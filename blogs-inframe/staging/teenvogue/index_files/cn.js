/*global CN:true, Option, jQuery, escape, unescape, console, alert, swfobject */ /* for jsLint */
...
CN.site = (function() {
    ...
    return {
         ...
         * @type string
         * Note:  We sometimes need to know CN.site.name before CN.site.init() so don't blow away
         *        the value by setting it to ''.  Check if set first.
         */
        name : (typeof(CN.site) ==! "undefined" && CN.site.name)? CN.site.name : 'teenvogue',
        ...
        */
         ...
         * @type boolean
         */
        noads : !!CN.url.params('cno.ads.disable') && !this.cnd,
         ...
         * @type String
         */
        testads : CN.url.params('cno.ads.testads') && !this.cnd,

        /**
         * Site time stamp.
         * @description Sets time stamp on CN.site obj. primarily for profiling purpose
         * @type number
         */
...
CN.callwhen = {
    ...
    run : function(funcs,target,state){
        var i,
            ...
            funcThis,
            funcObj,
            loadHandler = function(fObj, fThis) {
                return function() {
                    fObj.func.apply(fThis,fObj.params || []);
                };
            };
        ...
        if ((state==="before" || !state) && jQuery.isArray(funcs.callbefore)){
            for (i=0,len=funcs.callbefore.length; i<len; i++) {
        ...
        /* Run through the callback queue */
            for (i=0, len=funcs.callback.length; i<len; i++) {
                if (CN.isObject(funcs.callback[i])){
                    ...
                    if(jQuery.isFunction(funcObj.func)) {
                        target.bind(funcObj.event || 'load',loadHandler(funcObj, funcThis));
                    }
                }
            }
        }
    ...
    return {
        ...
        */
        ...
        set : function(name, value, options) {
            ...
            // There are some cases (user service) where we don't want to encode the cookie value.
            var cookie = [name + '=' + (options.encode === false ? value : encodeURIComponent(value))];
...
CN.date = (function() {
    var
        ...
        months = {
            ...
            es : {
                 ...
                 * @type array
                 */
                $long: ['enero','febraro','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','deciembre']
            }
        },
        ...
        days = {
            ...
            es: {
                 ...
                 * @type array
                 */
                $long: ['el domingo','el lines','el martes','el mi&eacute;rcoles','el jueves','el viernes','el s&aacute;bado']
            }
        },
    ...
    return {
        ...
        getMonthNames : function(options) {
            options = options || {};
            return months[options.lang || 'en']['$' + (options.form || 'long')] || [];
        },
        ...
        */
        getDayNames : function(options){
            options = options || {};
            return days[options.lang || 'en']['$' + (options.form || 'long')] || [];
        },
...
CN.frame = (function($) {
    var
        ...
        resize_private = function(e) {
            ...
            try {
                ...
                return CN.debug.user(eFrame, [frame, frame.id]);
            }

            if (typeof e.data === 'undefined') {
                $('iframe', body).bind('load', { use: frame }, this);
            }
    ...
    return {
        ...
        bindResize : function(frame) {
            $(frame).bind('load', resize_private);
            return this;
        },
        ...
        resize : function(frame) {
            $(frame).bind('load', resize_private).triggerHandler('load');
            return this;
        }
    };
}(jQuery));
    ...
    return {
        ...
        swf : function(args) {
            if (typeof swfobject === 'undefined') {
                ...
                $.getScript('/etc/designs/foundation/clientlibs/cn.common/misc/swfobject.js', function() {
...
CN.page = (function() {
    return {
         ...
         */
        section : function() {
             return ((CN.Author.Util.getContentPath().split('/')[3] || '').match(/^[^\.]*$/) || [''])[0];
        },
         ...
         */
        subsection : function() {
            return ((CN.Author.Util.getContentPath().split('/')[4] || '').match(/^[^\.|(\d{4})]*$/) || [''])[0];
        },
...
CN.world = (function($) {
    ...
    var
        states = {
            ...
            us : {
                desc: 'state',
                code: ['AL','AK','AZ','AR','CA','CO','CT','DE','DC','FL','GA','HI','ID','IL','IN','IA','KS','KY','LA','ME','MD','MA','MI','MN','MS','MO','MT','NE','NV','NH','NJ','NM','NY','NC','ND','OH','OK','OR','PA','RI','SC','SD','TN','TX','UT','VT','VA','WA','WV','WI','WY','AE','AA','AP'],
            ...
            ca : {
                desc: 'province',
                code: ['AB','BC','MB','NB','NL','NT','NS','NU','ON','PE','QC','SK','YT'],
                name: ['Alberta','British Columbia','Manitoba','New Brunswick','Newfoundland and Labrador','Northwest Territories','Nova Scotia','Nunavuta','Ontario','Prince Edward Island','Quebec','Saskatchewan','Yukon']
            }
        },
    ...
    return {
        ...
        setState : function() {
            ...
            if (!(states.hasOwnProperty(selection))) {
                ...
                choice = stateField.children("[selected]").val() || false;
                stateField.empty();
                stateField.removeAttr('disabled')[0][0] = new Option(states.msg + ' ' + states[selection].desc, '');
                $.each(states[selection].code, function(i, val) {
                    stateField[0][i + 1] = new Option(states[selection].name[i], val);
    ...
    for (i = 1, il = arguments.length; i < il; i++) {
        ...
        if (inter.constructor !== CN.Interface) {
            if (jQuery.browser.safari && jQuery.browser.version < 500) {
                ...
                CN.debug.info("Safari 2 check in CN.Interface.ensureImplements");
            } else {
...
CN.dom.storage.activateElement = function(el, storage, klass) {
    ...
    if (el !== null && el.getElementsByTagName('a').length > 0) {
        ...
        if (jQuery.browser.msie || jQuery.browser.safari) {
            if (jQuery.browser.version < 9) {
                jQuery('.media-thumbimg').each(function(){
                    var elementSel = jQuery(this),
                        elementId = jQuery(this).attr('id'),
                        elementClass = jQuery(this).attr('class'),
                        newDiv = jQuery('<div id="'+ elementId +'" class="'+ elementClass +'"></div>');
                    elementSel.before(newDiv);
                    newDiv.append(elementSel.children());
                    elementSel.remove();
                });
            }
        ...
        newEl.className = this.tempClass;
...
CN.dom.storage.deactivateElement = function(el, storage, klass) {
    ...
    if (el !== null && el.getElementsByTagName(this.innerTag).length > 0) {
...
/**
 * @description Simply expose CN.profiler with an options to create initial time
 * stamp, func. to set time stamps further during processing, finally provides
 * array of contextes and time stamps;
 * @todo Improve usage by hiding initial timestamp to private method;
 */

CN.profiler = ( function() {
    var
        stamp = 0,
        progress = 0,
        report = [];
    ...
    return {
         ...
         * @param   {string} [context] Descriptive informaton about time stamp
         * @return {Object} CN.profiler
         */
        setStamp: function(context) {
            progress = (stamp > 0) ? Math.abs((new Date().getTime() - CN.site.timestamp) - stamp) : Math.abs(new Date().getTime() - CN.site.timestamp);
            stamp = Math.abs(new Date().getTime() - CN.site.timestamp);
            report.push({context: context, stamp: stamp, progress: progress});
            return this;
        },
        /**
         * getReport.
         * @description Gets time stamps report
         * @return [Array] Array of contexts and time stamps
         */
        getReport: function() {
            return report;
        }
...
/**
 * HTML5 CORS functions.  At this current date and time, jQuery does not support it so
 * we have to build our own request method to support IE8+.  The public method request
 * will return a jQuery.Deferred object to allow callbacks to be added.
 */
CN.cors = (function() {
    var
        xhr;
    ...
    function createCORSRequest(method, url) {
        if (typeof window.XDomainRequest !== 'undefined') {
            ...
            try {
                ...
                CN.debug.warn('CORS is not supported by your browser.', [xhr, method, url, e]);
                xhr = false;
                return xhr;
            }
        }
        else if (typeof window.XMLHttpRequest !== 'undefined') {
            // XHR for Chrome/Safari/Firefox/IE10
            ...
            // CORS not supported.
            CN.debug.warn('CORS is not supported by your browser.', [xhr, method, url]);
            xhr = false;
        }
        return xhr;
    }

    function makeCorsRequest(config) {

        var
            def = new jQuery.Deferred();

        xhr = createCORSRequest((config.method || 'GET'), config.url);
        ...
        xhr.onload = function() {
            ...
            def.resolveWith((config.context || window), [response, "success"]);
        };

        xhr.onerror = function() {
            def.reject();
            CN.debug.error('CORS request failed', arguments);
        };
...
CN.debug = (function() {
    var
        ...
        show = function(type, msg, args) {
            ...
            if (CN.site.env != null && CN.site.env === 'PROD' && !CN.site.debug) {
                return;
            }
            ...
            if (typeof console === 'object') {
                func = console[t.f] || console.info;

                args.unshift(t.msg, msg);

                for (i = 0; i < args.length - 1; i += 2) {
                    part = args.splice(0, i + 1);
                    part.push(' :: ');
                ...
                if (console.firebug) {
                    ...
                    if (CN.isUndefined(console[t.f])) {
                        t.f = 'log'; // Default back to log level.
                    }
                    console[t.f](args);
                }
            }
        ...
        app : function() {
            ...
            return {
                ...
                setLevel : function(type, env) {
                    ...
                    env = (env && eEnv.hasOwnProperty(env)) ? env : 'DEV';
                    options[env] = type;
                    return this;
                },

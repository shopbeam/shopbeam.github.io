...
PEOPLE.recirc = {
    ...
    recirccallback : function(pos) { // PEOPLE.recirccallback, used in new global footer and article right-rail
        ...
        if ( ($('body').hasClass('article')) || ($('body').hasClass('permalink')) ){
...
PEOPLE.twitterPopup = function(e){
	...
	/*
	if ($('body').hasClass('article')) {tweetVia = 'PeopleMag'; isArticle = true;}
	...
	if ($('body#babies').hasClass('permalink')) {tweetVia = 'CBBvips'; isArticle = true;}
	...
	*/
	...
	else if (url.match('celebritybabies')) {
		tweetVia = 'CBBvips';
	}
...
// Set sitename based on whether we are using an iPad or not.
if ( navigator.userAgent.match('iPad') ) {
    peo_sitename = '3475.peo_tablet';
} else {
...
function postToTwitter(u,t){
	...
	tweetURL = 'http://twitter.com/share?url='+u + '&text=' + tweetTitle + '&related=PeopleMag,StyleWatchMag,PeoplePets,CBBvips&via=' + tweetVia;
...
var pageLoadFunctions = function() {
    ...
    if ( typeof( PEOPLE.Home ) != 'undefined' ) {
...
if ( tiiCookieGet('PEO_MOBILE') == 'false' ) {
    $(document).ready(function() {
        $('#footer .links2').append('<p style="padding:25px 0;">View PEOPLE.com in: <a href="http://www.people.com/people/static/mobile/h/remove/a.html" style="color:#067AB4;font-weight:bold;">Mobile</a> | <span style="color:#000;font-weight:bold">Classic</span></p>');
    });
}
...
$(document).ready(function() {
    ...
    // Simple Reach
    if ( location.href.match(/\/article\//) && $('meta[name=DATE_PUBLISHED]').length ) {
        ...
        __reach_config = {
            ...
            tags: ($('meta[name=CATEGORY_NAME]').attr('content')).split(',')
    ...
    // Buzzfeed
    if ( (location.href.match(/\/people\/article\//)) || (location.href.match(/\/people\/package\/article\//)) ) {
        BF_WIDGET_JS=document.createElement("script");
        BF_WIDGET_JS.type="text/javascript";
        BF_WIDGET_SRC="http://ct.buzzfeed.com/wd/UserWidget?u=people.com&to=1&or=vb&wid=1&cb=" + (new Date()).getTime();
        ...
        function trackSocial() {
            ...
            if (typeof (twttr) != 'undefined') {
                twttr.events.bind('follow', function (event) {
                    if (event.data.screen_name === 'peoplemag') {

...
var TIICONSTANTS = {
    ...
    'bk_disabled_root_domains': 'eskyguide.com,executivetravelmagazine.com',
    'bk_id': '13731',
    'bk_pixel_limit': '6',
...
function _Tii_revsciinit() {
    //name: Time Inc (Direct_US) PQ Test NEW 
    //created: 09/12/2013 12:48
    var cb = new Date().getTime();
    var asiPqTag = false;
    revscicounter = 0;
    if((typeof(TGX_SITE_CONFIG) != "undefined" && TGX_SITE_CONFIG.pggateway == true) || typeof(TGX_SITE_CONFIG) == "undefined") {
        try {
...
function TiiAdrevssciwrite() {
    if (typeof (asiPlacements) === 'undefined' && revscicounter < 3) {
        revscicounter++;
        revscitimer = window.setTimeout(TiiAdrevssciwrite, revscicounter * 40);
        return revscitimer;
    } else if (typeof asiPlacements !== 'undefined') {
        try {
            for(var p in asiPlacements) {
                if(asiPlacements[p]['default']&&asiPlacements[p]['default'].key){
                    window["ASPQ_"+p]="PQ_"+p;
                }
            }
        }
        catch(e) {

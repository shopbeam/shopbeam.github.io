...
// Chartbeat startup variable
var _sf_startpt=(new Date()).getTime();

function getDateCurrent (lang) {
    var monthNames = "";
  ...
  if (lang == "_en") {
      document.write(monthName + " " + day + ", " + year);
        ...
        document.write(day + " de " + monthName + ", " + year);
    }
}
...
function setMakeThisMyHomepage (obj)
{ 
    var bname = navigator.appName;
  if (bname == "Microsoft Internet Explorer"){
      obj.style.behavior='url(#default#homepage)';
      obj.sethomepage('http://www.peopleenespanol.com');
  }
  else {
...
function closeB(){
  ('#instructions, #overlay').css("display", "none");
}
...
function showPCF(){
  ...
  ('#mf').addClass('navr');
  ('#ms').addClass('nav');
  ('#pcf_content').css('display', 'block');
  ('#mf_content, #ms_content').css("display", "none");
}
...
function showMF(){
  ...
  ('#mf').addClass('nav_on');
  ('#ms').addClass('nav');
  ('#pcf_content, #ms_content').css("display", "none");
  ('#mf_content').css('display', 'block');
}
function showMS(){
  ('#pcf').addClass('nav');
  ('#mf').addClass('navl');
  ('#ms').addClass('nav_on');
  ('#pcf_content, #mf_content').css("display", "none");
  ('#ms_content').css('display', 'block');
}
function showRollover(counter1, counter2) {
  ('#counter1').css("display", "none");
  ('#counter2').css('display', 'block');
}
function hideRollover(counter1, counter2) {
  ('#counter1').css("display", "block");
  ('#counter2').css('display', 'none');
}
function showHPoverlay() {
  ('#rolloverImage').css('display', 'none');
  ('#overlayImage').css("display", "block");
}
function hideHPoverlay() {
  ('#rolloverImage').css("display", "block");
...
if ((navigator.userAgent.match('iPhone')) || (navigator.userAgent.match('iPad')) || (navigator.userAgent.match('iPod')) || (location.search.indexOf('ipad=true') > -1)) {
            ...
            tii_callFunctionOnWindowLoad(function(){
                        ...
                        for(i=0;i<objecttags.length;i++){
                                    ...
                                    objecttags[i].innerHTML = '<div class="removedFlash"><table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0"><tr><td valign="middle">Este contenido no se puede <br />ver en este dispositivo</td></tr></table></div>'

                        }

            });

             document.write('<style>embed{display:none;}.removedFlash{font:12px/14px Arial;text-transform:none;color:#000;border:1px solid #ccc;background:#f0f0f0;text-align:center;height:100px;}.adCM300x250 .discl{display:none}</style>');
...
addtwitter = function() {
  // add to estilo page with twitterfeed
  if (!document.getElementById('twitterfeed') && !$('body').hasClass('page-estilo') && !document.getElementById('twitterfeed-moms-babies')) return false;
       var s = document.createElement('script');
             s.type = 'text/javascript';
             s.src = '/sites/all/themes/peopleenespanol/js/twitter.js?c';
       document.getElementsByTagName('body')[0].appendChild(s);
    };
  tii_callFunctionOnWindowLoad(addtwitter);
...
/*-----------------------------------------------------------------------------*/
function TiiBcLcDcTracker (omniAdSiteName,omniAdZone) {var defaultFlg = 'false';if (omniAdZone == 'default') {omniAdZone = 'video_main_bc_lightningcast';defaultFlg = 'true';}var bcLCDCTmpPixel = new Image();bcLCDCTmpPixel.src = 'http://ad.doubleclick.net/ad/'+omniAdSiteName+'/'+omniAdZone+';sz=1x1;ord='+Math.ceil(1+1E12*Math.random());return 'Tracking successful - adSiteName="'+omniAdSiteName+'" ,adZone="'+omniAdZone+ '" defaultFlg="'+defaultFlg+'"'; }    
function TiiBrightcovePlayer() {this.cfg = new Array();this.flashUrl = "/web/tii/shared/swf/BrightcoveWrapper.swf";this.flashUrl = "/shared/static/swf/BrightcoveWrapper.swf";this.bgcolor = "#ffffff";this.cfg["objectId"] = "bcVideoPlayer";this.cfg["divId"] = "";this.cfg["testmode"] = "";this.cfg["autostart"] = false;this.cfg["lctracking"] = "";this.cfg["adsitename"] = "";this.cfg["lcadzone"] = "";this.setParam = TiiBcSetParam;this.write = TiiBcWrite;}
function TiiBcSetParam(key, value) {this.cfg[key] = value;}
function TiiBcWrite() {var fo = new FlashObject(this.flashUrl, this.cfg["objectId"], this.cfg["width"], this.cfg["height"], 8, this.bgcolor);fo.addParam("allowScriptAccess", "always");fo.addParam("menu", "false");fo.addParam("quality", "high");fo.addParam("bgcolor", this.bgcolor);fo.addParam("loop", "false");fo.addParam("wmode", "opaque");fo.addVariable("account", this.cfg["account"]);fo.addVariable("channel", this.cfg["siteId"]);fo.addVariable("prop16", this.cfg["channel"]);fo.addVariable("playerwidth", this.cfg["width"]);fo.addVariable("playerheight", this.cfg["height"]);fo.addVariable("playerid", this.cfg["playerId"]);fo.addVariable("videoid", this.cfg["videoId"]);fo.addVariable("lineupid", this.cfg["lineupId"]);fo.addVariable("autostart", this.cfg["autostart"]);fo.addVariable("lctracking", this.cfg["lctracking"]);fo.addVariable("adsitename", this.cfg["adsitename"]);fo.addVariable("lcadzone", this.cfg["lcadzone"]);fo.addVariable("objectid", this.cfg["objectId"]);fo.addVariable("adserverurl", this.cfg["adServerUrl"]);if (this.cfg["testmode"] != "") {fo.addVariable("testmode", this.cfg["testmode"]);}fo.altTxt = "";if (this.cfg["divId"] != "") {fo.write(this.cfg["divId"]);} else {fo.write();}}



   

 var adConfig = new TiiAdConfig("3475.pesp");
 adConfig.setCmSitename("cm.pesp");
 adConfig.setRevSciTracking(true);
 
var PESP = window.PESP || {};

/*PESP.attachHomePageCMLinks = function(e) {
        //adds CM tracking to every home page link
        var elem = (e.target.nodeName.toLowerCase() != 'a') ? $(e.target).parent('a') : $(e.target);
        var CMLinksSingle = 'N0, N1, N2, N3, N4, N5, N6, N7, N8, N9, N10, LMH1, LMH2, LMH3, UH1_H, UH1_P, UH1_T, UH1_CA, UH2_H, UH2_P, UH2_CA, UH3_H, UH3_P, UH3_CA, UH4_H, UH4_P, UH4_CA, UH5_H, UH5_P, UH5_CA, UH6_H, UH6_P, UH6_CA, UH7_H, UH7_P, UH7_CA, UH8_H, UH8_P, UH8_CA, UH9_H, UH9_P, UH9_CA, UH_CA, NML1_H, NML1_P, NML1_CA, NML2_H, NML2_P, NML2_CA, NML3_H, NML3_P, NML3_CA, NML4_H, NML4_P, NML4_CA, NML5_H, NML5_P, NML5_CA, C1_P1, C1_CA1, C1_H1, C1_T1, C1_CAT1, C1_P2, C1_CA2, C1_H2, C1_T2, C1_CAT2, C1_P3, C1_CA3, C1_H3, C1_T3, C1_CAT3, C2_H, C2_T, C2_P, C2_CA, C3_H, C3_T, C3_P, C3_CA, C4_H, C4_T, C4_P, C4_CA, C5_H, C5_T, C5_P, C5_CA, R1_H, R1_T, R1_P, R1_CA, R2_H, R2_T, R2_P, R2_CA, R3_H, R3_T, R3_P, R3_CA, R4_H, R4_T, R4_P, R4_CA, GMV1_H, GMV1_P, GMV1_CA, GMV2_H, GMV2_P, GMV2_CA, GMV3_H, GMV3_P, GMV3_CA, GMV4_H, GMV4_P, GMV4_CA, GMV5_H, GMV5_P, GMV5_CA';
...
PESP.attachArticlePageCMLinks = function(e) {
        //adds CM tracking to every home page link
        var elem = (e.target.nodeName.toLowerCase() != 'a') ? $(e.target).parent('a') : $(e.target);
        var CMLinksSingle = 'N1, N2, N3, N4, N5, N6, N7, N8, N9, N10, ART_PAG_TOP_PREV, ART_PAG_TOP_NEXT, ART_SM_COMM, ART_SM_FB, ART_SM_TW, ART_SM_MYSP, ART_SM_EMAIL, ART_PAG_BOT_PREV, ART_PAG_BOT_NEXT, ART_REL_ART_1, ART_REL_ART_2, ART_REL_ART_3, ART_REL_ART_4, ART_REL_ART_5, ART_REL_ART_6, ART_REL_GAL_1, ART_REL_GAL_2, ART_REL_GAL_3, ART_REL_GAL_4, ART_REL_GAL_5, ART_REL_GAL_6, ART_TodosLados_MAG, ART_TodosLados_MOV, ART_TodosLados_BOL, ART_TodosLados_TW, ART_TodosLados_FB, ART_TodosLados_MAGCA, ART_ULT_HORA_H1, ART_ULT_HORA_H2, ART_ULT_HORA_H3, ART_ULT_HORA_H4, ART_ULT_HORA_H5, ART_ULT_HORA_CA, ART_TF1_P, ART_TF1_H, ART_TF1_CA, ART_TF2_P, ART_TF2_H, ART_TF2_CA, ART_TF3_P, ART_TF3_H, ART_TF3_CA, ART_TF4_P, ART_TF4_H, ART_TF4_CA, ART_TF5_P, ART_TF5_H, ART_TF5_CA, ART_MAS1_P, ART_MAS1_H, ART_MAS1_CA, ART_MAS2_P, ART_MAS2_H, ART_MAS2_CA, ART_MAS3_P, ART_MAS3_H, ART_MAS3_CA, ART_MAS4_P, ART_MAS4_H, ART_MAS4_CA';
        if (CMLinksSingle.match(elem.attr('id'))) {
            var eltitle = (elem.attr('title').length  > 0 && elem.attr('title') != '') ? elem.attr('title') : elem.text();
            linkTrackArt(elem.attr('id'),eltitle);
        }
    };
...
PESP.Omniture = {
  ...
  // mapped in global namespace because g:plusone callback is unable to take PESP namespace
  omnitureRecordGooglePlus = PESP.Omniture.recordGooglePlus;


PESP.writeGoogleAd = function(width, height, channel, maxads){
  ...
  if(channel === 'gallery'){
    ...
    google_ad_client = 'ca-timeinc-peoplees-photo';

  }else{

    google_ad_client = 'ca-timeinc-peoplees-bah';
...
var google_ad_request_done = function(google_ads) { 
    ...
    s += ' <p class="adtext"><a href="' + google_info.feedback_url + '" target="new">Anuncios Google</a></p>'
    ...
    for (var i=0; i < google_ads.length; i++) {
      ...
      + ' <span class="line1"><a href="' + google_ads[i].url + '" target="new">' + google_ads[i].line1 + '</a></span><br/>'
  
      + ' <span class="line2">' + google_ads[i].line2  + '</span> <span class="line3">' +  google_ads[i].line3 + '</span><br/>'
  
      + ' <span class="line4"><a href="' + google_ads[i].url + '" target="new">' + google_ads[i].visible_url + '</a></span>'
...
(function(){
    ...
    var info = {
        siteName : getMetaOg('og:site_name'),
        trackURL : getClickURL(),
        trackTitle : encodeURIComponent($("meta[name='og:title']").attr("content")), //getClickTitle(),
        via : ' @PeopleEnEspanol'
    };
    ...
    var tools = {
        facebook : {
            ...
            postUrl : "http://www.facebook.com/sharer.php?u="+info.trackURL+"&t="+info.trackTitle,
            width : 655,
            height : 430
        },
        twitter : {
            label : "Twitter",
            postUrl : "http://twitter.com/home?status="+info.trackTitle +" "+ info.trackURL + info.via,
            width : 600,
            height : 450
        }
    };
    ...
    $(document).ready(function(){
        $('.share-tools .icon').click(function(){
            var shareName = $(this).attr('data-share');
                var newWinSetting = 'toolbar=0,status=0,height='+PESP.shareTools.tools[shareName].height+',width='+PESP.shareTools.tools[shareName].width+',scrollbars=yes,resizable=yes';

            if (typeof tools[shareName] !== 'undefined') {
                window.open(PESP.shareTools.tools[shareName].postUrl, 'Share', newWinSetting);
            }
        });
    });
    PESP.shareTools = {};
    PESP.shareTools.info = info;
    PESP.shareTools.tools = tools;
})();
...
PESP.appendAdScript = function(config) { // id required. w AND h OR multi required.
    var targetDivScript,
...
document.domain = 'peopleenespanol.com';

    ...
    Ctrls.Home = function() {
        ...
        function moveStuff(image, speed){
            ...
            if (top <= scrolled + winHeight) {
                top = "translate3d(0," + Math.floor(-(top - scrolled) / speed) + "px,0)";
                img.css({
        ...
        function parallax() {
            ...
            moveStuff(birdMedium, 5 );
            moveStuff(birdSmall,  10);
        }


        // Init
        build_home_slideshow();
        win.on("load resize", home_gallery);
        if(!navigator.userAgent.match(/(iPhone|iPod|iPad)/i)){ win.on("scroll resize", parallax); }

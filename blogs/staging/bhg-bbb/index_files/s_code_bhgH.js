/**
 * Script provided by Meredith to track users authenticated
 * on a partner site.
 */
...
function isMember() {
	var selected_cookie = getCookie("mdp_partner");

	if (selected_cookie) {
		var cookieData = selected_cookie.split("|");
		if (cookieData[3] == "Y") {
			return "true";
		}
	}
...
var mds = mds || {};
mds.s=s_gi(s_account);
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
mds.s.charSet="ISO-8859-1";
/* Conversion Config */
mds.s.currencyCode="USD";
/* Link Tracking Config */
mds.s.trackDownloadLinks=true;
mds.s.trackExternalLinks=true;
mds.s.trackInlineStats=true;
mds.s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,ram,swf,rm,wm";
mds.s.linkLeaveQueryString=true;
mds.s.linkTrackVars="None";
mds.s.linkTrackEvents="None";
mds.s.visitorNamespace="meredith";
mds.s.server="bhg.com";
mds.s.prop7=isMember();
/* Plugin Config */
mds.s.usePlugins=true;
function s_doPlugins(s) {
    /* Add calls to plugins here */
    mds.s.campaign=mds.s.getQueryParam('ordersrc');
    if(!mds.s.campaign)
    	mds.s.campaign=mds.s.getQueryParam('sssdmh');
    mds.s.eVar8=mds.s.getQueryParam('psrc');
    if (!mds.s.eVar15)
    	mds.s.eVar15=mds.s.getQueryParam('esrc');
    if (!mds.s.eVar35)
    	mds.s.eVar35=mds.s.getQueryParam('socsrc');
    mds.s.eVar9=mds.s.pageName;
    mds.s.eVar14=mds.s.channel;
}
mds.s.doPlugins=s_doPlugins;

/*********Media Module Calls**************/
mds.s.loadModule("Media");

mds.s.Media.autoTrack = false;
mds.s.Media.trackVars = "eVar29,eVar40,eVar41,eVar42,eVar43,prop43,events";
mds.s.Media.trackEvents="event70,event71,event72,event73,event74,event75,event76";
mds.s.Media.trackMilestones = "25,50,75";
mds.s.Media.segmentByMilestones = true;
mds.s.Media.trackUsingContextData = true;
mds.s.Media.contextDataMapping = {
...
/****************************** MODULES *****************************/
/* Module: Media */
mds.s.m_Media_c="var m=s.m_i('Media');if(m.completeByCloseOffset==undefined)m.completeByCloseOffset=1;if(m.completeCloseOffsetThreshold==undefined)m.completeCloseOffsetThreshold=1;m.cn=function(n){var m="
 ...
 */
mds.s.getQueryParam=new Function("p","d","u",""
+"var s=this,v='',i,t;d=d?d:'';u=u?u:(s.pageURL?s.pageURL:''+s.wd.loc"
+"ation);u=u=='f'?''+s.gtfs().location:u;while(p){i=p.indexOf(',');i="
+"i<0?p.length:i;t=s.p_gpv(p.substring(0,i),u);if(t)v+=v?d+t:t;p=p.su"
+"bstring(i==p.length?i:i+1)}return v");
mds.s.p_gpv=new Function("k","u",""
+"var s=this,v='',i=u.indexOf('?'),q;if(k&&i>-1){q=u.substring(i+1);v"
+"=s.pt(q,'&','p_gvf',k)}return v");
mds.s.p_gvf=new Function("t","k",""
...
mds.s.trackingServer="metrics.bhg.com";
mds.s.trackingServerSecure="smetrics.bhg.com";
